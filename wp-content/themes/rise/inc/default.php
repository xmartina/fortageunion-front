...
if (!class_exists('\UiCore\Core')) {
	...
	function uicore_widgets_init()
	{
		register_sidebar(
			array(
				'name' => esc_html__( 'Left Sidebar', 'rise' ),
				'id'            => 'left-sidebar',
		...
		register_sidebar(
			array(
				'name' => esc_html__( 'Right Sidebar', 'rise' ),
				'id'            => 'right-sidebar',
				'before_widget' => '<div class="uicore-sidebar-element">',
				'after_widget' => '</div>',
				'before_title' => '<h4 class="ui-widget-title">',
	...
	{
		wp_enqueue_style('rise', get_stylesheet_uri(), array(), '1.0.0');
	...
	function uicore_pagination_style($default){
		$args = array(
			'before'           => '<nav aria-label="'.esc_attr__('Posts navigation', 'rise') .'" class="ui-pagination ui-pages"> <ul>',
			'after'            => '</ul></nav>',
			...
			'separator'        => ' ',
			'nextpagelink'     => esc_html__( 'Next page', 'rise' ),
			'previouspagelink' => esc_html__( 'Previous page', 'rise'  ),
			'pagelink'         => '%',
			'echo'             => 1,
		);
	...
	function uicore_pagination()
	{
		...
		$args = wp_parse_args(
			array(
				'mid_size'           => 2,
				'prev_next'          => true,
				'prev_text'          => esc_html__('', 'rise'),
				'next_text'          => esc_html__('', 'rise'),
				'screen_reader_text' => esc_html__('Posts navigation', 'rise'),
				'type'               => 'array',
...
?>
	...
	function uicore_page_title()
	{
		...
		if (is_search()) {
            echo sprintf(
                esc_html__('Search Results for: %s', 'rise'),
                '<span>' . get_search_query() . '</span>'
			);
		} else if (is_home()) {
			echo esc_html__('Latest Posts', 'rise');
	...
	if (class_exists('WooCommerce')) {
		...
		function uicore_woo_widgets_init()
		{
			register_sidebar(
				array(
					'name' => esc_attr__( 'Product Sidebar', 'rise' ),
					'id'            => 'product-sidebar',
			...
			register_sidebar(
				array(
					'name' => esc_attr__( 'Shop Sidebar', 'rise' ),
					'id'            => 'shop-sidebar',
					'before_widget' => '<div class="uicore-sidebar-element %2$s">',
					'after_widget' => '</div>',
					'before_title' => '<h4 class="ui-widget-title">',
